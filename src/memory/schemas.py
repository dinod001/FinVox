...
class Episode:
    ...
    turn_count: int = 0
    ttl_at: Optional[float] = None  # epoch seconds for expiration
...
class EpisodicStore(Protocol):
    """
    Episodic memory store (Supabase Postgres + pgvector).

    Stores full conversation session summaries with metadata and vector embeddings.
    """
