...
Two collections:
    ``finvox_kb``  — RAG knowledge-base chunks for SME Financial data (persistent)
...
def get_qdrant_client() -> QdrantClient:
    ...
    log.info("Connected to Qdrant Cloud at {}", QDRANT_URL)
    return _qdrant_client
    ...
    for i in range(0, len(chunks), batch_size):
        ...
        for chunk, vec in zip(batch_chunks, batch_embeds):
            ...
            for k, v in chunk.items():
                if k not in ("text", "url", "source", "title", "strategy", "chunk_index", "document_id"):
                    payload[k] = v
        ...
        total += len(points)
        log.debug("Upserted batch {}–{} ({} points)", i, i + len(points), len(points))

    log.info("Upserted {} points into '{}'", total, collection_name)
    return total
...
def delete_chunks_by_document_id(
    ...
    try:
        ...
        log.info("Deleted all chunks for document_id '{}' from '{}'", document_id, collection_name)
        return True
